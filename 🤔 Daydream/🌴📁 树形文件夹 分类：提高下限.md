...
但一方面 [[PKM/Apps/Tana/🇵🇭🔨 Tana Philosophy#Tana 群友发言|它的 supertag 的 attr/property 继承逻辑 没有 tana 好]]
...
- 而 SiYuan 只能通过 [[🤔 Daydream/💌⭐ 思源 v1：SilverBullet#思源 缺乏 live query?|插件]] 来将 query 嵌入 某块。
  - 这一点 Orca 继承了 [[Logseq]]
    ...
    - 但我今天试了下（2025-12-09）
      - 不论是官方部署后的网站，还是 logseq 自身，似乎也都不卡了...
...
- [[Logseq]] 的 live query 可以自动刷新（且应该是事件触发的）
  - 也很容易部署成 site：https://docs.logseq.com/
    - [publish spa](https://github.com/logseq/publish-spa) #github
  - 似乎还有个 preview 的 db 版本：https://test.logseq.com/
    - 向 [tana 的 超级标签](https://discuss.logseq.com/t/logseq-db-unofficial-faq/32508/2?u=xczphysics) 看齐
    - database 版本 中，内存与磁盘的双向交互，只发生在 解释器 与 SQLite 之间，而 SQLite 单向自动导出 md 而无法逆流/导回 [logseq db unofficial faq](https://discuss.logseq.com/t/logseq-db-unofficial-faq/32508) #discuss #logseq
    - 这意味着 [[github 同步 md 文件没有作用了，只能同步 SQLite 文件，但由于 SQLite 加密了⚓|github 同步 md 文件没有作用了，只能同步 SQLite 文件，但由于 SQLite 加密了🔙]]${backRefs("github 同步 md 文件没有作用了，只能同步 SQLite 文件，但由于 SQLite 加密了")}（类似 思源）同步过去 也没有用
      - [will logseq sync be open source](https://discuss.logseq.com/t/will-logseq-sync-be-open-source/31903/13) #discuss #logseq
      - [logseq db unofficial faq](https://discuss.logseq.com/t/logseq-db-unofficial-faq/32508/32?u=xczphysics) #discuss #logseq
  - 以及 demo: https://demo.logseq.com/
    - [logseq db unofficial faq](https://discuss.logseq.com/t/logseq-db-unofficial-faq/32508/31?u=xczphysics) #discuss #logseq

    - 不过是 ==静态的==，非 app ？似乎并不。
    - 有反链，但丢失了许多 原生 CMDs ? 似乎并不。
      - 全局 Query 和 live query 都在
      - 其他 cmds (在左上角🔍中，不输入符号时) 也没有丢
        - `/` 选择/缩小查询范围
        - `shift + 回车` 在 [[side bar]] 中打开所选（作为 ==目录==! 天!）
          - 我以为 是将打开 [[side bar]] 前的页面作为 静态目录 side页
            - 跳转不发生在 side bar 页，而发生在原左侧，这个天才设计
            - 结果只是 pined wikis... 是 ==结构化的（非list）星标== 而已
              - 所有页面的 “目录” 都是一样的... 无语子...
          - 这个设计很好，就不用像 SB 一样...老是想着“回去 alt←”了...
          - 当然，像 Tana 或 tiddly wiki 或 思源 “分屏”开 tabs 也行..
      - 和 SB 一样，即使只读，也可以执行大多数 CMDs
    - 右上角 ... → `View as slides` 可以在 [[side bar]] 中打开 幻灯片
      - 可以像 B 站 `绿导师原谅你了` 的 Slides 一样
      - 有 ←→↑↓ 方向箭头，滑动 slides（左右 ←→ 是大跨步 跳章节）
      - #💡 我才知道 这样的 Slides 的本质 反映了 树状的结构...
    - ==鼠标悬停预览== `[[]]` 都在的...
  - Logseq 的 这个：设计...哲学... 也应该借鉴！
    - 这个页面（注意其中的 `/page/`）https://docs.logseq.com/#/page/the%20basics%20of%20block%20references 下，有一个 header（注意其中的 `/block/`）https://docs.logseq.com/#/page/the%20basics%20of%20block%20references/block/what%20are%20block%20references%3F，这个 header 下有一个 最底层的块（但==没有== `/block/`） https://docs.logseq.com/#/page/692d8498-4312-44bc-aab6-eafd21d2e29f
      - 由于它不是 header，所以 Url 直接就是 一串 uid... 666
