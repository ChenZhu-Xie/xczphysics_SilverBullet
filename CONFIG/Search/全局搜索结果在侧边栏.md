...
command.define {
    ...
    run = function()
        local keyword = editor.prompt("🔍 侧边栏搜索", "输入关键词...")
        if keyword and keyword:trim() ~= "" then
            keyword = keyword:trim()
            local results, matchCount, pageCount = searchGlobalOptimized(keyword)
            ...
            if matchCount > 0 then
                ...
                table.insert(output, string.format("*%d 处匹配，%d 个页面*", matchCount, pageCount))
                table.insert(output, "")
                for _, line in ipairs(results) do
                    table.insert(output, line)
                end
                local markdown = table.concat(output, "\n")
                editor.showPanel('rhs', 1, markdown.markdownToHtml(markdown))
            else
                editor.flashNotification('未找到: ' .. keyword, 'warn')
            end
        end
    end,
    key = "Ctrl-Shift-f",
