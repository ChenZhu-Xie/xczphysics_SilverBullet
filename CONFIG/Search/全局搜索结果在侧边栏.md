...
-- Highlight styles for different keywords (循环使用)
local highlightStyles = {
    function(kw) return "==" .. kw .. "==" end,           -- 黄色高亮
    function(kw) return "**" .. kw .. "**" end,           -- 粗体
    function(kw) return "*" .. kw .. "*" end,             -- 斜体
    function(kw) return "**==" .. kw .. "==**" end,       -- 粗体+高亮
    function(kw) return "*==" .. kw .. "==*" end,         -- 斜体+高亮
    function(kw) return "`" .. kw .. "`" end,             -- 代码
    function(kw) return "**`" .. kw .. "`**" end,         -- 粗体+代码
    function(kw) return "*`" .. kw .. "`*" end,           -- 斜体+代码
}
            ...
            if content and containsAllKeywords(content, keywords) then
                ...
                if #contexts > 0 then
                    ...
                    for i, ctx in ipairs(contexts) do
                        local formatted = string.format("%d. …%s…", i, ctx.snippet)
...
command.define {
    name = "Global Search",
    run = function()
        local keyword = editor.prompt("🔍 Global Search (space = AND)", "")
